...
def parse_json(json_path: str) -> list:
    ...
    def parse_event(event: dict) -> list:
        """ Parsing event dict """
        nonlocal att, num_att, json_name, json_path
        ...
        if "band_name" in event_keys:
            ...
            if "attendance" not in event_keys:
                event["attendance"] = str(json_path)
            else: 
    ...
    if len(venue) == 0:
        ...
        venue_stats["is_indie"].append(MISSING)
        venue_stats["attendance"].append(MISSING)
        venue_stats["invalid"].append(MISSING)
    else:
    ...
    # Add mapper
    fill_attendance_map[str(json_path)] = att
...
def main(set_path: str):
    ...
    ########################### ANSWER C ##################################
    C_ans = stats_df.city.value_counts().idxmax()
    print(C_ans)

    ########################### ANSWER D ##################################
    # print(stats_df[stats_df.is_indie == True].groupby('city').agg({"attendance": ['sum']}).sort_values(('attendance', 'sum'), ascending=False))
...
if __name__ == "__main__":
    ...
    # Get input
    set_path = input() # BUG
    #set_path = "/home/grbic/Desktop/PSIML:8/B/public/sets/8"
